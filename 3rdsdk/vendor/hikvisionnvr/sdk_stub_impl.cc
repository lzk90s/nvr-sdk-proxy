...
int32_t SdkStubImpl::QueryDevice(std::vector<Device> &devices) {
    ...
    BOOL succeed = NET_DVR_GetDVRConfig(handle_, NET_DVR_GET_IPPARACFG_V40, 0, &ipAccessCfg, sizeof(NET_DVR_IPPARACFG_V40), &dwReturned);
    if (!succeed) { //不支持ip接入,9000以下设备不支持禁用模拟通道
        NET_DVR_DEVICECFG devCfg = {0};
        DWORD bytesreturned      = 0;
        CHECK(NET_DVR_GetDVRConfig(handle_, NET_DVR_GET_DEVICECFG, 0, &devCfg, sizeof(NET_DVR_DEVICECFG), &bytesreturned), "NET_DVR_GetDVRConfig");
        ...
        for (int i = 0; i < MAX_IP_CHANNEL; i++) {
            if (ipAccessCfg.struStreamMode[i].uGetStream.struChanInfo.byEnable && ipAccessCfg.struIPDevInfo[i].byEnable) { // ip通道在线
...
int32_t SdkStubImpl::DownloadRecordByTime(const std::string &devId, const TimePoint &startTime, const TimePoint &endTime, OnDownloadData onData,
    ...
    // start download
    CHECK(NET_DVR_SetPlayDataCallBack_V40(context->downloadId, downloadDataCallBack, (void *)context.get()), "NET_DVR_SetPlayDataCallBack");
    ...
    context->timer.add(
        interval,
        [playContextPtr, this](CppTime::timer_id) {
    ...
    STUB_LLOG_INFO("Downloading file for dev {} between {} and {}, downloadId {}", devId, startTime.ToString(), endTime.ToString(),
                   context->downloadId);
...
int32_t SdkStubImpl::GetFtp(const std::string &devId, FtpInfo &ftpInfo) {
    ...
    CHECK(NET_DVR_GetDeviceConfig(handle_, NET_DVR_GET_FTPCFG_V40, 1, &ftpType, sizeof(NET_DVR_FTP_TYPE), &result, &ftpCfg, sizeof(NET_DVR_FTPCFG_V40)),
          "CLIENT_GetDevConfig");
...
int32_t SdkStubImpl::SetFtp(const std::string &devId, const FtpInfo &ftpInfo) {
    ...
    if (!ftpInfo.dir0.empty() && !ftpInfo.dir1.empty()) {
        ftpCfg.byDirLevel = 2;
        // byTopDirMode: 0x1- 使用设备名，0x2- 使用设备号，0x3- 使用设备ip地址，0x4- 使用监测点，0x5- 使用时间(年月)，0x6- 使用自定义，0x7-
        // 违规类型，0x8- 方向，0x9- 地点，0xa- 栋号单元号，0xb- 期号
        ftpCfg.byTopDirMode = 6;
        // bySubDirMode: 0x1- 使用通道名，0x2- 使用通道号，0x3- 使用时间(年月日)，0x4- 使用车道号，0x5- 使用自定义，0x6- 违规类型，0x7- 方向，0x8-
        // 地点，0x9- 车位编号，0xa- 栋号单元号，0xb- 门口机编号，0xc- 设备编号
    ...
    CHECK(NET_DVR_SetDeviceConfig(handle_, NET_DVR_SET_FTPCFG_V40, 1, &ftpType, sizeof(NET_DVR_FTP_TYPE), &result, &ftpCfg, sizeof(NET_DVR_FTPCFG_V40)),
...
std::string SdkStubImpl::getChannelName(int channelIdx) {
    ...
    DWORD dwReturned;
    if (TRUE != NET_DVR_GetDVRConfig(handle_, NET_DVR_GET_PICCFG_V40, channelIdx, &picCfg, sizeof(NET_DVR_PICCFG_V40), &dwReturned)) {
